        ...
        with self._conn() as conn:
            ...
            conn.execute(
                ...
                UPDATE vehicles SET
                    ...
                    mileage = CASE WHEN ? IS NOT NULL THEN ? ELSE mileage END,
                    condition = CASE WHEN ? IS NOT NULL AND ? <> '' THEN ? ELSE condition END,
                    engine_hp = CASE WHEN ? IS NOT NULL AND ? <> '' THEN ? ELSE engine_hp END,
                    transmission = CASE WHEN ? IS NOT NULL AND ? <> '' THEN ? ELSE transmission END,
                    engine = CASE WHEN ? IS NOT NULL AND ? <> '' THEN ? ELSE engine END,
                    torque = CASE WHEN ? IS NOT NULL AND ? <> '' THEN ? ELSE torque END,
                    towing_capacity = CASE WHEN ? IS NOT NULL AND ? <> '' THEN ? ELSE towing_capacity END,
                    flat_tow = CASE WHEN ? IS NOT NULL AND ? <> '' THEN ? ELSE flat_tow END,
                    vdp_enriched_at = ?
                WHERE id = ?
                """,
                (
                    data.get("stock_number"), data.get("stock_number"), data.get("stock_number"),
                    data.get("image_url"), data.get("image_url"), data.get("image_url"),
                    data.get("mileage"), data.get("mileage"),
                    data.get("condition"), data.get("condition"), data.get("condition"),
                    data.get("engine_hp"), data.get("engine_hp"), data.get("engine_hp"),
                    data.get("transmission"), data.get("transmission"), data.get("transmission"),
                    data.get("engine"), data.get("engine"), data.get("engine"),
                    data.get("torque"), data.get("torque"), data.get("torque"),
                    data.get("towing_capacity"), data.get("towing_capacity"), data.get("towing_capacity"),
                    data.get("flat_tow"), data.get("flat_tow"), data.get("flat_tow"),
                    now,
                    vehicle_id,
                ),
            )
